...
class SearchBloc extends Bloc<SearchEvent, SearchState> {
  ...
  SearchBloc({
    ...
    on<OnActivateManuelMarkerEvent>((event, emit) => emit(state.copyWith(displayManualMarker: true)));
    on<OnDesactivateManuelMarkerEvent>((event, emit) => emit(state.copyWith(displayManualMarker: false)));
  ...
  Future<RouteDestination> getCoorsStartToEnd( LatLng start, LatLng end) async{
    final trafficResponse = await trafficService.geteCoorsStartToEnd(start, end);
